...
def problem_two(request):
  ...
  instructors = Instructor.objects.all().filter(hire_date__year__lt = 2010).order_by()
...
def problem_three(request):
  ...
  courses = Course.objects.all().filter(instructor_id__in = "2")

  # instructors = Instructor.objects.filter(in__contains = 2)
  # courses = Course.objects.filter(name__in = instructors)
...
def problem_five(request):
  ...
  students = Student.objects.all().create(first_name = "Micha", last_name = "Singleton", year = 2022, gpa = 4.0)
